...
> This is a C# variation of my other project <https://github.com/iblazhko/cqrs-fsharp/>.

Example of a simple inventory management system implemented in C# using DDD,
CQRS, and Event Sourcing.

This project is using functional programming and an excellent library
...
- _API_ sends Commands to _Application_ (command API), and uses
    _ProjectionStore_ to retrieve view models (query API).
- _Ports_. Abstractions for event store, projections store, and message bus.
- _Adapters_. Implementations of _Ports_ abstraction that (typically) use
    external services, e.g. MartenDB or RabbitMQ.
- _CLI_: example of an external client that interacts with
    the system via _API_.
